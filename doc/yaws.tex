...
Note that in practice, your callback module might receive a
\verb+text+ or \verb+binary+ message \emph{after} receiving a
\verb+close+ message. For example, the Chrome browser, specifically
version 17.0.963.46 but perhaps other versions as well, can send a
\verb+close+ message ahead of other messages even if the client code
invokes \verb+send()+ to send its messages before calling
\verb+close()+ on its websocket.
...
\item \verb+{close, Reason}+ --- close the connection and exit the
  ...
  \verb+Reason+ should be the atom \verb+normal+. Callback handlers
